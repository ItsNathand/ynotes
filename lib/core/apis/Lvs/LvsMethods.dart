...
class LvsMethod {
  ...
  LvsMethod(this.client, this._offlineController);
  ...
  Future<dynamic> fetchAnyData(
    ...
    if (connectivityResult == ConnectivityResult.none && !isOfflineLocked) {
      ...
      try {
        await onlineFetchWithLock(onlineFetch, lockName,
  ...
  Future<List<Discipline>> grades() async {
    ...
    if (periods != null) {
      await Future.forEach(periods, (PronotePeriod period) async {
        ...
        var temp = await request(
            "DernieresNotes", PronoteDisciplineConverter.disciplines,
            data: jsonData, onglet: 198);
    ...
    await DisciplinesOffline(_offlineController)
        .updateDisciplines(listDisciplines);
  ...
  Future<dynamic> request(String functionName, Function? converter,
