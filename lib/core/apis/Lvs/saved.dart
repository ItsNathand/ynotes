...
class LVSClient extends SessionClient {
  ...
  Future<http.Response> get(Uri url,
    ...
    print('calleed get for' + url.toString());
    if (token) {
...
class HwClient extends SessionClient {
  ...
  Future<List> init(Map<String, dynamic> credentials) async {
    print(credentials);
    var entry_url =
        //await credentials['method'].get(Uri.parse('https://www.google.com'));

        await Function.apply(credentials['method'], [credentials['pos']],
                credentials['named'])
            .data
            .location;
    print(entry_url.body);
/*     Response redirect = await this.get(Uri.parse(entry_url));
  ...
  Future<http.Response> post(Uri url,
    ...
    return await super
        .post(url, body: body, headers: headers, baseUrl: baseUrl);
  }
}
